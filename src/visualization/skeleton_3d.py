...
class Skeleton3D:
    ...
    def create_frame(
        self, landmarks_3d: np.ndarray, title: str = "3D Skeleton"
    ) -> go.Figure:
        """
        ...
        fig.update_layout(
            ...
            scene=dict(
                xaxis=dict(
                    ...
                    autorange="reversed",  # Invert X axis for correct orientation
                ),
                yaxis=dict(
                    title="Y",
                    showgrid=self.show_grid,
                    backgroundcolor=self.background_color,
                    autorange="reversed",  # Invert Y axis for correct orientation
                ),
                zaxis=dict(
                    title="Z",
                    showgrid=self.show_grid,
    ...
    def create_animation(
        ...
        if landmarks_sequence.shape[1] != 33:
            raise ValueError(
                f"Expected 33 landmarks, got {landmarks_sequence.shape[1]}"
            )
        ...
        fig.update_layout(
            ...
            # Preserve user interactions (camera rotation) during animation
            uirevision="constant",
            scene=dict(
                camera=dict(
                    center=dict(x=0, y=0, z=0),
    ...
    def create_comparison(
        ...
        for col_idx, (landmarks, label) in enumerate(
            zip(landmarks_list, labels), start=1
        ):
            if landmarks.shape[0] != 33:
                raise ValueError(
                    f"Expected 33 landmarks for {label}, got {landmarks.shape[0]}"
                )
    ...
    def save_figure(
        self, fig: go.Figure, output_path: str, format: str = "html"
    ) -> None:
        """
